...
<div class="max-w-3xl">
    <form method="POST" action="{{ route('admin.posts.update', $post) }}" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <textarea name="excerpt" rows="2" class="w-full rounded-lg bg-slate-900 border border-slate-700 px-4 py-2 text-sm text-slate-100 focus:border-blue-500 focus:outline-none">{{ old('excerpt', $post->excerpt) }}</textarea>
        </div>

        <div>
            <label class="block text-xs font-medium text-slate-400 mb-1">Conteúdo *</label>
            <textarea name="body" rows="12" required class="w-full rounded-lg bg-slate-900 border border-slate-700 px-4 py-2 text-sm text-slate-100 font-mono focus:border-blue-500 focus:outline-none">{{ old('body', $post->body) }}</textarea>
        </div>

        <div>
            <label class="block text-xs font-medium text-slate-400 mb-1">Imagem de capa</label>
            @if ($post->cover_image)
                <img src="{{ Storage::url($post->cover_image) }}" alt="" class="mb-2 h-24 rounded object-cover">
            @endif
