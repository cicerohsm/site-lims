...
@section('content')
<div class="max-w-3xl">
    <form method="POST" action="{{ route('admin.publications.update', $publication) }}" class="space-y-5">
        @csrf @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="authors" value="{{ old('authors', $publication->authors) }}" required
                   class="w-full rounded-lg bg-slate-900 border border-slate-700 px-4 py-2 text-sm text-slate-100 focus:border-blue-500 focus:outline-none">
        </div>
        ...
        <div class="grid grid-cols-3 gap-4">
            <div>
                ...
                <input type="number" name="year" value="{{ old('year', $publication->year) }}" min="1900" max="2100" required
                       class="w-full rounded-lg bg-slate-900 border border-slate-700 px-4 py-2 text-sm text-slate-100 focus:border-blue-500 focus:outline-none">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="doi" value="{{ old('doi', $publication->doi) }}"
                       class="w-full rounded-lg bg-slate-900 border border-slate-700 px-4 py-2 text-sm text-slate-100 focus:border-blue-500 focus:outline-none">
            </div>
        </div>
        ...
        <div>
            ...
            <input type="text" name="venue" value="{{ old('venue', $publication->venue) }}"
                   class="w-full rounded-lg bg-slate-900 border border-slate-700 px-4 py-2 text-sm text-slate-100 focus:border-blue-500 focus:outline-none">
        </div>

        <div>
            <label class="block text-xs font-medium text-slate-400 mb-1">URL</label>
            <input type="url" name="url" value="{{ old('url', $publication->url) }}"
                   class="w-full rounded-lg bg-slate-900 border border-slate-700 px-4 py-2 text-sm text-slate-100 focus:border-blue-500 focus:outline-none">
        </div>

        <div>
            <label class="block text-xs font-medium text-slate-400 mb-1">Resumo</label>
            <textarea name="abstract" rows="5" class="w-full rounded-lg bg-slate-900 border border-slate-700 px-4 py-2 text-sm text-slate-100 focus:border-blue-500 focus:outline-none">{{ old('abstract', $publication->abstract) }}</textarea>
        </div>
