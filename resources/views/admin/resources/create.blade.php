...
@section('content')
<div class="max-w-2xl">
    <form method="POST" action="{{ route('admin.resources.store') }}" enctype="multipart/form-data" class="space-y-5">
        @csrf
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full rounded-lg bg-slate-900 border border-slate-700 px-4 py-2 text-sm text-slate-100 focus:border-blue-500 focus:outline-none">{{ old('description') }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div class="flex items-center gap-2 pt-5">
                <input type="checkbox" name="is_public" value="1" id="is_public" @checked(old('is_public', true)) class="rounded">
        ...
        <div>
            <label class="block text-xs font-medium text-slate-400 mb-1">Arquivo (upload)</label>
